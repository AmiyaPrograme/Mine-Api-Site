...
var validApiKeys = ["maskser","izumi-v3"]
...
if (!code) return res.json({status: false,  message: "No Code Given"})
var JavaScriptObfuscator = require('javascript-obfuscator');
var obfuscationResult = JavaScriptObfuscator.obfuscate(code,value);
res.json({status: 200,  code: `${obfuscationResult.getObfuscatedCode()}`})
} catch (e) {
res.json({status: 404,  code: `${e}`})
}
})
...
router.get('/randomvideo/lovevideo', async (req, res, next) => {
      const apikey = req.query.apikey;
	...
	let {result} = await fetchJson('https://raw.githubusercontent.com/mask-sir/api.mask-ser/main/Sts.json')
	let random = result [Math.floor(Math.random() * result.length)]
        var resultt = await getBuffer(random)
...
router.get('/anime/naruto', async (req, res, next) => {
	...
	let { result } = await fetchJson('https://raw.githubusercontent.com/mask-sir/api.mask-ser/main/Naruto.json')
	let random = result[Math.floor(Math.random() * result.length)]
...
router.get('/anime/anivideo', async (req, res, next) => {
        const apikey = req.query.apikey;
	...
	let { result } = await fetchJson('https://raw.githubusercontent.com/mask-sir/api.mask-ser/main/AnimeVideos.json')
	let random = result[Math.floor(Math.random() * result.length)]
...
router.get('/randomvideo/sexy', async (req, res, next) => {
      const apikey = req.query.apikey;
...
let sts = await fetchJson('https://raw.githubusercontent.com/mask-sir/api.mask-ser/main/Asupan.json')
...
router.get('/get/session', async (req, res, next) => {
    ...
    if (!session) {
        return res.status(400).json({
            status: false,
            message: "Bad Request: 'session' query parameter is required"
        });
    }

    try {
     
        const data = await fetchJson(`https://pastebin.com/raw/${session}`);
...
let { result } = await fetchJson('https://raw.githubusercontent.com/mask-sir/api.mask-ser/main/Loli.json')
	let random = result[Math.floor(Math.random() * result.length)]
        var resultt = await getBuffer(random)
	res.set({'Content-Type': 'image/png'})
	res.send(resultt)
...
router.get('/quote', async (req, res, next) => {
  ...
  const jsonResponse = await fetchJson("https://raw.githubusercontent.com/mask-sir/api.mask-ser/main/quotes.json");
    const randomIndex = Math.floor(Math.random() * jsonResponse.length);
    const randomQuote = jsonResponse[randomIndex];
